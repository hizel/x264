#!/bin/bash
# PAFF hardware-decoder interop smoke test.
#
# Encodes the PAFF stream set (paff + 14-config matrix + rc + one
...
# Same stream set as tools/test_paff.sh -- keep in sync.
...
echo "== PAFF hardware interop: backend=$BACKEND md5=$MD5_POLICY =="
"$FFMPEG" -version | head -1 | sed 's/^/ffmpeg: /'

# --- stream set, verbatim from tools/test_paff.sh ---------------
