...
# Synthetic dissolve clip for the weightb gate: two 50 fps progressive sources crossfaded, then tinterlace.  As with
...
# log-linear interpolation (Bjontegaard-style): integrate
...
# Wide-range PAFF round-trips (field-geometry MV limits): the geometry
...
# Synthetic vertical-motion clip (sign/parity coverage): large motion in
...
cmd_paff() {
    ...
    # Active sliding window -- --ref 2 forces eviction every pair
    # (exercises the between-pass eviction), TFF + BFF.
    ...
    # The X264_REF_MAX=16 FIELD-ENTRY ceiling -- --ref 8 pairs => 16 field
    # entries (capped), mixed parity, TFF + BFF.  (--ref 16 pairs would set
...
# The 14-config B-field regression matrix
...
# Rate-control round-trips (the CRF/2-pass/CBR × TFF/BFF gate beyond
...
# VBV+B round-trips are covered by earlier validation.
cmd_rc() {
    make_clip "$WORKDIR/clip_${WIDTH}x${HEIGHT}.yuv"
...
# Weightb quality measurement (measured and rejected): CRF sweep on the
