...
typedef struct
{
    ...
    struct /* struct for future expansion */
    {
        ...
        int i_poc;
        /* PAFF: target field parity of this opcode (0=top, 1=bottom).
...
typedef struct
{
     ...
     * reads only its own pass's half.  Unused by
...
struct x264_t
{
     ...
     * weightp determinism fix).  The monolithic --threads
    ...
    /* PAFF: one-shot warning flag for ignored per-frame pic_struct
     * (pulldown) inputs, which PAFF cannot honour. */
    int b_paff_pic_struct_warned;

    /* PAFF: one-shot warning flag for the hard-clamped mmco[]
...
#define FDEC_STRIDE 32
            ...
            int i_fref[2];
            /* PAFF field pictures: the expanded L0 list interleaves
            ...
            /* PAFF: L1 counterparts of the L0 field-entry maps above,
