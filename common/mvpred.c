...
static int mb_predict_mv_direct16x16_temporal( x264_t *h )
{
    ...
    {
        /* PAFF: uniformly-field picture.  RefPicList1[0]
        ...
        /* PAFF: 8.4.1.2.4 single-field rule -- if the colocated field's
...
static ALWAYS_INLINE int mb_predict_mv_direct16x16_spatial( x264_t *h, int b_interlaced )
{
    ...
    {
        /* PAFF: uniformly-field picture.  The
        ...
        int col_parity = h->mb.pic.i_fref_parity_l1[0];
        /* PAFF: 8.4.1.2.4 single-field rule -- defensive; the L1
...
#undef SET_MVP
    ...
    /* temporal predictors */
    /* PAFF: i_ref[0] is per-parity.  Under FIELD_PIC i_ref is
