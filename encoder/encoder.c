...
#undef PAFF_PUSH
     ...
     * because this expansion equals the decoder's default list. */
    if( i_list )
                             ...
                             x264_picture_t *pic_out )
{
    ...
    *pp_nal = NULL;
    ...
    if( h->param.b_paff )
    {
        ...
        /* PAFF: the pair is encoded synchronously -- both field passes run
         ...
         * by reference_build_list + x264_reference_build_list_optimal:
        ...
        /* PAFF: the pair-level marking commands (reference_hierarchy_reset +
        ...
        /* PAFF: pair-folded L0 ref counts, accumulated per pass below (each
        ...
        /* PAFF: snapshot the pair-level slice type so the per-pass Ip
        ...
        for( int pass = 0; pass < 2; pass++ )
        {
             ...
             * eviction.  The idr_pic_id xor-undo hack is
            ...
            if( pass )
            {
                /* PAFF: field 2 must not duplicate field 1's marking
            ...
            /* PAFF: fdec->i_poc stays the even frame-level POC for the whole
            ...
            /* PAFF: expand the pair-level lists into per-field
            ...
            {
                /* PAFF: build the per-field L1.  The decoder's default L1 is
            ...
            /* PAFF: slice_header_init (encoder.c:161) read the colocated
            ...
            /* PAFF: build the bipred tables for this field pass.  The
            ...
            /* PAFF: pic_timing SEI per coded field (Table D-1 mandates
            ...
            /* PAFF: per-field bit accounting (stat bits of this coded field);
            ...
            if( !pass )
            {
                ...
                paff_sync_references( h );
                /* PAFF: the decoded-reference-picture marking process runs after
                ...
                if( h->fdec->b_kept_as_ref )
                {
                    ...
                    {
                        /* PAFF sliding-window marking: mirror the decoder's field-1
         ...
         * the trim opcodes must still evict the trimmed pairs. */
        h->sh.i_mmco_command_count = mmco_command_count;
        h->sh.i_mmco_remove_from_end = mmco_remove_from_end;
        /* PAFF: restore the pair-level slice/nal type mutated for the
         ...
         * even frame-level POC.  Under PAFF fdec->i_poc is never mutated per
        ...
        /* publish the pair-folded L0 ref counts, overwriting the
