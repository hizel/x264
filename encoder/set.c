...
void x264_sps_init( x264_sps_t *sps, int i_id, x264_param_t *param )
{
     ...
     * first field (the Ip structure), so the stream DOES use one
    ...
    /* DPB accounting is in slot units: one complementary field pair =
     ...
     * is stored, but the between-pass eviction in the PAFF driver mirrors
...
void x264_sei_pic_timing_write( x264_t *h, bs_t *s, int i_field_tick_offset )
{
    ...
    if( sps->vui.b_nal_hrd_parameters_present || sps->vui.b_vcl_hrd_parameters_present )
    {
        /* PAFF: each field is its own access unit, removed from the CPB
    ...
    if( sps->vui.b_pic_struct_present )
    {
        ...
        if( h->param.b_paff )
            /* PAFF: emitted per coded field; the value comes from the
...
void x264_sei_dec_ref_pic_marking_write( x264_t *h, bs_t *s )
{
    ...
    {
        /* PAFF: the original picture is a field pair and the repeated
