#!/bin/bash
# relax clip cells, appends to results.csv
FF=~/dev/FFmpeg/ffmpeg
cd /tmp/paff_mb
CRFS="18 23 28 33"

run_cell() {
    local src=$1 mode=$2 mb=$3 crf=$4 name="${src%.y4m}_${2}_${3}_${4}"
    local extra=""
    case $mode in
...
for mb in 1 0; do
    for crf in $CRFS; do
        run_cell relax_25i.y4m prog  $mb $crf
        run_cell relax_25i.y4m mbaff $mb $crf
        run_cell relax_25i.y4m paff  $mb $crf
        run_cell relax_25p.y4m prog  $mb $crf
    done
done
echo ALL DONE
