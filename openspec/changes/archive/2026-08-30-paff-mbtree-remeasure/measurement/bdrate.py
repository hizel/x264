...
rows = list(csv.DictReader(open('/tmp/paff_mb/results.csv')))
data = {}
for r in rows:
    secs = 8.0 if r['content'].endswith('_25p') else 16.0
    key = (r['content'], r['mode'], r['mbtree'])
    data.setdefault(key, []).append((float(r['psnr_y']), int(r['bytes'])*8/secs/1000))

def g(content, mode, mb): return data[(content, mode, mb)]

for clip in ('hall', 'relax'):
    I, P = f'{clip}_25i', f'{clip}_25p'
    print(f"===== {clip} =====")
