# threaded-ratecontrol Specification

## Purpose

Frame-threaded encoding with VBV enabled must honour the
`doc/threads.txt` promise of byte-deterministic output at a fixed thread
count.  This capability defines how threaded VBV decisions are kept free
of timing-dependent state (live per-row state of in-flight frames on
other slots), and the determinism, invariance, and compliance guarantees
that result.
